/*Plagiarism detection using BagOfWords technique*/
...
int main(int argc, char const *argv[]){						//main takes command line argument, i.e., path of test files
	printf("Plagiarism check by Bag Of Words technique is done\n");
			...
			if(checkstatus[i][j]==0){
				if(i==j){percentage[i][j]=150.0;checkstatus[i][j]=1;}
					...
					if(strlen(text1)==0||strlen(text2)==0){percentage[i][j]=200;checkstatus[i][j]=1;percentage[j][i]=200;checkstatus[j][i]=1;}
						...
						int no1=numberOfWords(text1);						//to find no.of words i.e., no. of spaces
						int no2=numberOfWords(text2);
						
						int m1=dictonaryMaker(text1,dict1,no1);						//dictonary containing unique words and word count
						int m2=dictonaryMaker(text2,dict2,no2);

						/*to print the made dictonaries i.e., bags of words
						 printf("{");for(int i=0;i<m1-1;i++){printf("%s:%d, ",dict1[i].uniqueword,dict1[i].uniquewordcount);}printf("%s:%d}",dict1[m1-1].uniqueword,dict1[m1-1].uniquewordcount);printf("\n");
						 printf("{");for(int i=0;i<m2-1;i++){printf("%s:%d, ",dict2[i].uniqueword,dict2[i].uniquewordcount);}printf("%s:%d}",dict2[m2-1].uniqueword,dict2[m2-1].uniquewordcount);printf("\n");
						*/

						percentage[i][j]=calculate(m1,dict1,m2,dict2);checkstatus[i][j]=1;						//calculate is a function containing all the required calculations
	...
	fprintf(fp,"Plagiarism check by BagOfWords technique\n\n");
