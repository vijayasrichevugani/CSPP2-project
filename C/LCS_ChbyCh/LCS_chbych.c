...
int main(int argc, char const *argv[]){						//main takes command line argument, i.e., path of test files
	printf("Plagiarism check by LCS chbych technique is done\n");
			...
			if(checkstatus[i][j]==0){
				if(i==j){percentage[i][j]=150.0;checkstatus[i][j]=1;}
					...
					if(strlen(text1)==0||strlen(text2)==0){percentage[i][j]=200;checkstatus[i][j]=1;percentage[j][i]=200;checkstatus[j][i]=1;}
						...
						int lcs=LCS(text1,text2);						//to find Longest Common Substring

						int len1=strlen(text1);						//no. of characers available in text1
						int len2=strlen(text2);						//no. of characers available in text2
	...
	fprintf(fp,"Plagiarism check by LCS chbych technique\n\n");
